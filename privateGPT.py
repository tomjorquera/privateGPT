...
llamacpp_n_gpu_layers = os.environ.get('MODEL_N_GPU_LAYERS', None)

# Warn user of options combinations that do not make sense
if llamacpp_n_gpu_layers is not None and not (use_gpu and model_type == "LlamaCpp"):
    warn("Setting 'MODEL_N_GPU_LAYERS' has no effect unless 'USE_GPU' == True and 'MODEL_TYPE' == 'LlamaCpp'")
...
def main():
    ...
    match model_type:
        case "LlamaCpp":
            n_gpu_layers = llamacpp_n_gpu_layers if use_gpu else None
            if use_gpu and n_gpu_layers is None:
                n_gpu_layers = -1
